(() => {
  ...
  function clearLines() { let count=0; for(let y=ROWS-1;y>=0;y--) if(grid[y].every(Boolean)) { grid.splice(y,1); grid.unshift(Array(COLS).fill(null)); count++; y++; } if(count) { lines+=count; score += [0,100,300,500,800][count] * (1+Math.floor(lines/10)); celebrate.textContent = count===4 ? 'FOUR LINES!??!' : ['NICE ROW','DOUBLE TROUBLE','TRIPLE VISION'][count-1]; celebrate.classList.remove('show'); void celebrate.offsetWidth; celebrate.classList.add('show'); updateText(); reality('LINE CLEAR — LOOK WHAT YOU DID'); } }
